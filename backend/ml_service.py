...
class MLService:
    ...
    def load_models(self) -> Tuple[int, List[Tuple[str, str]]]:
        """Load all ML models and return success count and failures"""
        loaded_count = 0
        failed_models = []
    ...
    def load_feature_ranges(self) -> bool:
        ...
        if os.path.exists(x_test_path):
            try:
                ...
                sample_data = pd.read_csv(x_test_path, index_col=0, nrows=settings.SAMPLE_SIZE)
    ...
    def get_feature_importance(self, model_name: str) -> Optional[Dict]:
        ...
        feature_importance = dict(zip(settings.FEATURE_NAMES, importance.tolist()))
